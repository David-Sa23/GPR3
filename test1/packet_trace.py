...
matplotlib.use('Agg')  # Backend para gerar imagens sem interface gráfica

app = Flask(__name__)

# Variáveis globais controlando estado
latest_predictions = []
sniffing = False
start_time = None
...
def predict_realtime(packet):
    ...
    # Calcula frequência baseada no intervalo de tempo entre pacotes
    if start_time is None:
        start_time = current_time

    elapsed = max(current_time - start_time, 0.001)  # Evita divisão por zero
    freq = 1 / elapsed
    start_time = current_time
    ...
    # Armazena apenas os últimos 100 pacotes para evitar crescimento infinito
    ...
    if len(latest_predictions) > 250: # Limita a 250 entradas
        # Remove o primeiro elemento se a lista exceder o tamanho máximo
...
def get_metrics():
    ...
    # Pode não haver classe '1', tratar caso
    metrics = {
