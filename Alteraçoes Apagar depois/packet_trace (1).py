...
def predict_realtime(packet):
    ...
    # Inicializa como None, caso o pacote não tenha TCP nem UDP
    src_port = None
    dst_port = None
    ...
    if len(latest_predictions) > 1000: # Limita a 1000 entradas
...
def export_csv():
    ...
    si = io.StringIO()
    cw = csv.writer(si)
    cw.writerow(['Origem', 'Porta Origem', 'Destino', 'Porta Destino', 'Tamanho (bytes)', 'Protocolo', 'Frequência (pkt/s)', 'Predição'])
